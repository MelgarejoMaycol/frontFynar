...
export function VerificationPendingPage() {
  ...
  return (
    <section className={styles.statusPage}>
      ...
      <Card>
        <p>
          Abre el enlace del correo para activar tu cuenta. También revisa la
          carpeta de spam.
        </p>
        {!params.get('email') && (
          <label>
        ...
        {resend.isSuccess && (
          <p className={styles.success} role="status">
            Si el registro continúa pendiente, enviaremos un nuevo enlace.
          </p>
        )}
        {resend.isError && (
        ...
        <div className={styles.statusActions}>
          <Button
            disabled={!email || remaining > 0}
            loading={resend.isPending}
            onClick={() =>
              resend.mutate(email, { onSuccess: () => setRemaining(60) })
            }
          >
  ...
  return (
    <section className={styles.statusPage}>
      ...
      <PageHeader
        ...
        description={
          verify.isPending
            ? 'Espera un momento mientras validamos el enlace.'
            : verified
        ...
        <Button
          onClick={() =>
            window.location.assign(
              verified ? '/login' : '/verify-email/pending',
            )
          }
        >
          {verified ? 'Iniciar sesión' : 'Reenviar verificación'}
        </Button>
      )}
    </section>
  )
