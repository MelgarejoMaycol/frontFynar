...
export function BudgetDashboardWidget() {
  ...
  const relevant = [...query.data.items]
    .sort(
      (a, b) =>
        Number(b.progress.percentage) - Number(a.progress.percentage) ||
  ...
  return (
    <Card className={styles.widget}>
      <div className={styles.header}>
        <h2>Presupuestos</h2>
        {query.data.items.length > 3 && (
          <Link to="/app/budgets">Ver todos</Link>
        )}
      </div>
      {relevant.length === 0 ? (
            ...
            return (
              ...
              >
                <div>
                  ...
                  <span>{budget.progress.percentage} %</span>
                </div>
                <div
                  className={styles.progress}
                  role="progressbar"
                  aria-label={`${budget.name}: ${budget.progress.percentage} % utilizado`}
                  aria-valuemin={0}
                  aria-valuemax={100}
                ...
                <small>
                  {formatMoney(budget.progress.spent, budget.currency)} de{' '}
