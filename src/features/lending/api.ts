...
export const lendingApi = {
  summary: (workspaceId: string, signal?: AbortSignal) =>
    httpClient.get<ApiSuccess<LendingSummary>>(
  ...
  list: (
    ...
    return httpClient.get<ApiSuccess<LoanListItem[]>>(
      `${base(workspaceId)}/loans?${query}`,
      signal,
    )
  },
  get: (workspaceId: string, id: string, signal?: AbortSignal) =>
    httpClient.get<ApiSuccess<LoanDetail>>(
      `${base(workspaceId)}/loans/${id}`,
      signal,
    ),
  ...
  update: (workspaceId: string, loanId: string, input: UpdateLoanInput) =>
    httpClient.patch<ApiSuccess<LoanDetail>, UpdateLoanInput>(
      `${base(workspaceId)}/loans/${loanId}`,
      input,
    ),
  collect: (workspaceId: string, loanId: string, input: LoanPaymentInput) =>
    httpClient.post<ApiSuccess<LoanDetail>, LoanPaymentInput>(
      `${base(workspaceId)}/loans/${loanId}/payments`,
      input,
    ),
  pay: (
    workspaceId: string,
  ...
  reverse: (
    ...
    httpClient.post<ApiSuccess<LoanDetail>, { reason: string }>(
      `${base(workspaceId)}/loans/${loanId}/payments/${paymentId}/reverse`,
      { reason },
    ),
  archive: (workspaceId: string, loanId: string) =>
    httpClient.delete<
      ApiSuccess<{ id: string; archived: boolean; mode?: 'REVERSED' }>
    >(`${base(workspaceId)}/loans/${loanId}`),
}
